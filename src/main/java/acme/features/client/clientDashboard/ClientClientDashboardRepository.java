...
public interface ClientClientDashboardRepository extends AbstractRepository {
	...
	@Query("select avg(c.budget.amount) from Contract c where c.client.id = :clientId and c.published=true")
	Optional<Double> findAverageContractBudget(int clientId);

	@Query("select max(c.budget.amount) from Contract c where c.client.id = :clientId and c.published=true")
	Optional<Double> findMaxContractBudget(int clientId);

	@Query("select min(c.budget.amount) from Contract c where c.client.id = :clientId and c.published=true")
	Optional<Double> findMinContractBudget(int clientId);

	@Query("select stddev(c.budget.amount) from Contract c where c.client.id = :clientId and c.published=true")
